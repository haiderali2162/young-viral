...
var select2 = __webpack_require__(6);
...
function initToggleClass() {
  ...
  function close() {
    if (document.querySelector('.' + openedClass)) {
      document.querySelectorAll('.' + openedClass).forEach(function (item) {
...
// Select
function initSelect() {
  if ($('.js-select_default').length) {
    $('.js-select_default').select2({
...
// Swiper
function initSwiper() {
  if ($('.js-swiper__projects').length) {
    var swiper = new _swiper2.default('.js-swiper__projects', {
      spaceBetween: 20,
      slidesPerView: 'auto',
      speed: 300,
...
function initContactCaptcha() {
  $('.contacts__form [name="message"]').on('keyup', function () {
    $('.contacts__captcha').css('display', $(this).val().length > 0 ? 'flex' : 'none');
  });
}
...
function submitForm() {
  function preloadStart() {
    $('.js-form__standart').addClass('preloader');
  }

  function preloadEnd() {
    $('.js-form__standart').removeClass('preloader');
  }
  ...
  function success(result) {
    ...
    if (!result.error) {
      if (result.html) {
        $('.js-form__standart').html(result.html);
      }
    } else {
      console.error('Error');
    }
  ...
  document.addEventListener('submit', function (e) {
    if (e.target.tagName === 'FORM' && e.target.classList.contains('js-form__standart')) {
...
var serialize = exports.serialize = function serialize(obj, prefix) {
  var str = [];
  var p = void 0;
  for (p in obj) {
...
function getRequest(config) {
  ...
  if (config.url) {
    ...
    if ((typeof requestData === 'undefined' ? 'undefined' : _typeof(requestData)) === 'object') {
      var csrfParam = document.querySelector('[name="csrf-param"]');
      var csrfToken = document.querySelector('[name="csrf-token"]');
      if (csrfParam && csrfToken) {
        requestData[csrfParam.getAttribute('content')] = csrfToken.getAttribute('content');
      }
    }
    ...
    if (config.method.toLowerCase() === 'get' && requestData.length > 0) {
      // requestData аргументы вида a=1&b=2&c=3
    ...
    // The header must be after `.open()`, but before `.send()` (для безопастности)
    ...
    console.error('Error! Set url to ajax request!');
  }
}
...
function checkDevice() {
  ...
  var deviceValue = parseInt(getComputedStyle(el, null).getPropertyValue('z-index'), 10);
  if (deviceValue === 1) {
    returnValue = 'desktop';
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;/*! Select2 4.0.6-rc.1 | https://github.com/select2/select2/blob/master/LICENSE.md */!function(a){ true?!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(0)], __WEBPACK_AMD_DEFINE_FACTORY__ = (a),
